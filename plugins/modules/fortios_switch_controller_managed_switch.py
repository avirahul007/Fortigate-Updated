...
def filter_switch_controller_managed_switch_data(json):
    option_list = ['settings_802_1X', 'access_profile', 'custom_command',
                   ...
                   'dynamic_capability', 'dynamically_discovered', 'firmware_provision',
                   'firmware_provision_version', 'flow_identity', 'fsw_wan1_admin',
                   'fsw_wan1_peer', 'fsw_wan2_admin', 'fsw_wan2_peer',
                   'igmp_snooping', 'ip_source_guard', 'l3_discovered',
                   'max_allowed_trunk_members', 'mclag_igmp_snooping_aware', 'mirror',
                   'name', 'override_snmp_community', 'override_snmp_sysinfo',
                   'override_snmp_trap_threshold', 'override_snmp_user', 'owner_vdom',
                   ...
                   'switch_profile', 'switch_stp_settings', 'tdr_supported',
                   'type', 'version']
    dictionary = {}
...
def flatten_multilists_attributes(data):
    multilist_attrs = [[u'snmp_community', u'events']]
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_controller_managed_switch"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_managed_switch"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_controller_managed_switch")

        is_error, has_changed, result = fortios_switch_controller(module.params, fos, module.check_mode)
