    ...
    system_password_policy_guest_admin:
        ...
        suboptions:
            ...
            min_change_characters:
                description:
                    - Minimum number of unique characters in new password which do not exist in old password (This attribute overrides reuse-password if both
                       are enabled).
                type: int
...
def filter_system_password_policy_guest_admin_data(json):
    option_list = ['apply_to', 'change_4_characters', 'expire_day',
                   'expire_status', 'min_change_characters', 'min_lower_case_letter',
                   'min_non_alphanumeric', 'min_number', 'min_upper_case_letter',
                   'minimum_length', 'reuse_password', 'status']
    dictionary = {}
...
def system_password_policy_guest_admin(data, fos):
    ...
    system_password_policy_guest_admin_data = flatten_multilists_attributes(system_password_policy_guest_admin_data)
...
def fortios_system(data, fos):
    ...
    if data['system_password_policy_guest_admin']:
        ...
        fos._module.fail_json(msg='missing task body: %s' % ('system_password_policy_guest_admin'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_password_policy_guest_admin"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_password_policy_guest_admin"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_password_policy_guest_admin")
